...
int PreLoadBmp(char *FileName, int *Width, int *Height)
{
    ...
    size_t readed = fread(&FileHeader, sizeof(BMPFileHeader), 1, fh);
    (void) readed; // ignored
    ...
    readed = fread(&InfoHeader, sizeof(BMPInfoHeader), 1, fh);
    (void) readed; // ignored
...
void LoadBmpAsGray(char *FileName, int Stride, ROI ImSize, byte *Img)
{
    ...
    for (int i=ImSize.height-1; i>=0; i--)
    {
        ...
        {
            int r=0, g=0, b=0;
            size_t readed = fread(&b, 1, 1, fh);
            readed = fread(&g, 1, 1, fh);
            readed = fread(&r, 1, 1, fh);
            (void) readed; // ignored
